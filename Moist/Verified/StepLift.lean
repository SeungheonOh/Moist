...
/-- Helper: given a sub-computation that eventually halts, find the first
    ...
    same halted value. Also proves that the lifted computation reaches
    the same state. -/
...
/-- **Generalized Apply decomposition (halt case)**: if `Apply f x` halts
    ...
    **Proof sketch:**
    1. One mechanical step: `compute [] env (Apply f x)`
       → `compute [.arg x env] env f`.
    2. Rewrite as `liftState [.arg x env] (compute [] env f)`.
    3. Use stack lifting to evaluate `f` in isolation, obtaining `vf`.
    4. One mechanical step: `ret [.arg x env] vf`
       → `compute [.funV vf] env x`.
    5. Rewrite as `liftState [.funV vf] (compute [] env x)`.
    6. Use stack lifting to evaluate `x` in isolation, obtaining `vx`.
    7. The remaining steps from `ret [.funV vf] vx` give the application. -/
theorem apply_reaches (env : CekEnv) (f x : Term) (v : CekValue)
  ...
  obtain ⟨N, hN⟩ := hreach
  -- After 1 mechanical step: compute [arg x env] env f
  have hge1 : N ≥ 1 := by
    match N, hN with
...
theorem apply_reaches_error (env : CekEnv) (f x : Term)
  ...
  have hlift_f : State.compute [.arg x env] env f =
      liftState [.arg x env] (.compute [] env f) := by simp [liftState]
  rw [hlift_f] at hrest1
  -- Use liftedErrorValue to decompose the f-evaluation phase
...
/-- **Generalized Apply composition (synthesis direction)**: given that `f`
    ...
    This generalizes `beta_apply_from_inner` to arbitrary function terms. -/
theorem apply_compose (env : CekEnv) (f x : Term) (vf vx : CekValue) (s : State)
  ...
  obtain ⟨Kex, hKex⟩ := hx
  obtain ⟨Ka, hKa⟩ := happ
  -- 1 mechanical step: compute [] env (Apply f x) → compute [arg x env] env f
  have h1 : steps 1 (.compute [] env (.Apply f x)) =
      .compute [.arg x env] env f := by simp [steps, step]
  -- Identify as liftState
  have hlift_f : State.compute [.arg x env] env f =
      liftState [.arg x env] (.compute [] env f) := by simp [liftState]
  -- Find the first non-active step in f computation (bounded by Kef)
  ...
  -- Inner state at Kf is not error (since f eventually halts)
  ...
  have hlift_x : State.compute [.funV vf] env x =
      liftState [.funV vf] (.compute [] env x) := by simp [liftState]
  -- Find the first non-active step in x computation (bounded by Kex)
  ...
  -- Inner state at Kx is not error (since x eventually halts)
  ...
  -- Show vx' = vx by determinism
  have h_vx_eq : vx' = vx :=
    halt_value_unique' hKx_le h_inner_x_eq hKex
  rw [h_vx_eq] at h_lifted_x_eq
  -- Compose everything: 1 + Kf + 1 + Kx + 1 + Ka steps total
