...
deliberately treats every other SMT application as opaque:
the theorem below therefore does not depend on a typing assumption for the
open SMT expression language.
-/
