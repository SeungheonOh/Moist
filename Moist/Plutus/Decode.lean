...
def decodeBytestring (s : List Bool) : Option (List Bool × ByteString) := do
  let unpadded ← unpad s
  let (s', r)  ← decodeCStar unpadded
  .some (s', ByteArray.mk (r.toArray.map Char.toUInt8))
...
partial def decodeConstValue (s : List Bool) : BuiltinType → Option (List Bool × Const)
  ...
  | .AtomicType .TypeByteString     => Prod.map id .ByteString <$> decodeBytestring s
...
/- Decodes a UPLC term. -/
partial def decodeTerm (v : Version) : List Bool → Option (List Bool × Term)
  | false :: false :: false :: false :: s => Prod.map id .Var                          <$> decodeVar s
  | false :: false :: false :: true  :: s => Prod.map id .Delay                        <$> decodeTerm v s
  | false :: false :: true  :: false :: s => Prod.map id (.Lam 0) <$> decodeTerm v s
  | false :: false :: true  :: true  :: s => do
      let (s' , t₁) ← decodeTerm v s
      let (s'', t₂) ← decodeTerm v s'
      .some (s'', .Apply t₁ t₂)
  | false :: true  :: false :: false :: s => Prod.map id .Constant <$> decodeConst s
  | false :: true  :: false :: true  :: s => Prod.map id .Force   <$> decodeTerm v s
  ...
  | true  :: false :: false :: false :: s => do
      ...
      let (s'', l) ← decodeList (decodeTerm v) s'
      .some (s'', .Constr i l)
  | true  :: false :: false :: true  :: s => do
      let _        ← if v < .Version 1 1 0 then .none else .some ()
      let (s' , u) ← decodeTerm v s
      let (s'', l) ← decodeList (decodeTerm v) s'
      .some (s'', .Case u l)
  | _ => .none
...
def bitSequenceFromByteString : List Char → List Bool → Option (List Bool)
  ...
  | c :: t, acc =>
      ...
      let bcc := [b.getLsbD 7, b.getLsbD 6, b.getLsbD 5, b.getLsbD 4, b.getLsbD 3, b.getLsbD 2, b.getLsbD 1, b.getLsbD 0]
...
def foo : Program :=
  match decodeProgramFromHexString "0100004bd6f583006010282308854101" with
  | .some p => p
  | .none   => .Program (.Version 0 0 0) (.Error)

def testRountTrip (script: String) : Bool :=
match decodeProgramFromHexString script with
  | .some p => (encode_program p).toHexString == script
  | .none   => false

def aaa (a : Option Program) : Program :=
  match a with
  | .some p => p
  | .none   => .Program (.Version 0 0 0) (.Error)
